...
class BasicFlowConvNet(nn.Module):
    ...
    def __init__(
        ...
        hidden_channels: int,
        param_dims,
        context_dims: int = None,
        param_nonlinearities=None,
    ):
        super().__init__()
        ...
        self.seq1 = nn.Sequential(
            nn.Conv2d(
                in_channels + context_dims if context_dims is not None else in_channels,
                hidden_channels,
                kernel_size=3,
                padding=1,
            ),
    ...
    def forward(self, inputs, context=None):
        ...
        if not (self.context_dims is None) == (context is None):
            raise ValueError(
                "Given context does not match context dims: context: {} and context_dims:{}".format(
                    context, self.context_dims
                )
            )
        ...
        *batch_dims, h, w, c = inputs.size()
        ...
        if context is not None:
            ...
            context = context.view(*context.shape, 1, 1).expand(
                -1, -1, *outputs.shape[2:]
            )
        ...
        if self.param_nonlinearities is not None:
            if isinstance(self.param_nonlinearities, Iterable):
                outputs = tuple(
                    n(o) for o, n in zip(outputs, self.param_nonlinearities)
                )
            else:
